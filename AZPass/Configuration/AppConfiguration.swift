//
//  AppConfiguration.swift
//  AZPass
//
//  Created by Eric Webb on 9/25/18.
//  Copyright © 2018 Gluu. All rights reserved.
//

import UIKit


struct AppConfiguration {
    static let systemTitle = "AZPass"
    static let systemAlertIcon = UIImage(named: "icon_gluu_logo_nav") ?? UIImage()
    static let systemLogIcon = UIImage(named: "icon_azpass_logo_log_details") ?? UIImage()
    static let systemLogRedIcon = UIImage(named: "icon_azpass_logo_log_details_failed")  ?? UIImage()
    static let systemColor = UIColor.Gluu.red
}
